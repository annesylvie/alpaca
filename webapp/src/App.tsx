...
function App() {
    ...
    return (
        <div>
            {boxes.map(box => (<Box fieldName={box} />))}
            <FormCreator
...
function Box(props: {fieldName: string}) {
    return <div
        className="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center mx-2 my-2"
    > Name: {props.fieldName} </div>
}
...
export const FormCreator: React.FC<IFormProps> = (
    {showFields, setShowFields, boxes, setBoxes}: IFormProps
) => {
    return (
        showFields ?
            <Form setShowFields={setShowFields}
                boxes={boxes}
                setBoxes={setBoxes}
            />
            : <div>
